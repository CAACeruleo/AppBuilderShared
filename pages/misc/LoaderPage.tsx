...
export default function LoaderPage(
	props: Props & LoaderProps & {logoSrc?: string},
) {
	const {children, logoSrc = "/loading-logo.svg", ...rest} = props as any;
	const restDefault = useProps(
		"LoaderPage",
		defaultStyleProps,
		rest,
	) as StyleProps & LoaderProps;
	...
	const px =
		typeof restDefault.size === "number"
			? restDefault.size
			: (sizeMap[restDefault.size ?? "md"] ?? 128);
